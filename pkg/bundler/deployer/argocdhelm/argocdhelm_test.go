...
// TestGenerate_IgnoreComputeDomainCRDDiff asserts the computedomains
// ignoreDifferences stopgap generated by the argocd deployer (see
// pkg/bundler/deployer/argocd.TestGenerate_IgnoreComputeDomainCRDDiff)
// survives transformApplication's parse/remarshal into the argocd-helm
// child template unchanged. See NVIDIA/aicr#2546.
func TestGenerate_IgnoreComputeDomainCRDDiff(t *testing.T) {
	...
	rr.DeploymentOrder = []string{"gpu-operator", "nvidia-dra-driver-gpu"}
	...
	if !strings.Contains(got, "RespectIgnoreDifferences=true") {
		t.Errorf("gpu-operator child template must contain \"RespectIgnoreDifferences=true\":\n%s", got)
	}

...
Dependencies resolve through the Go module proxy; this project does not vendor them (see ADR-023). After changing imports, run `make tidy` and commit `go.mod` and `go.sum`. CI fails if the manifests are not tidy (`go mod tidy -diff`). `THIRD_PARTY_NOTICES.md` is not committed — it is generated by `make release` and published as a release asset — but the merge gate does verify on dependency changes that the generator still runs, since a release depends on it.
